...
/** TTL 是否表示"永不自动关闭" */
function isForever(ttl) {
  const s = String(ttl || '').trim().toLowerCase();
  return s === 'forever' || s === '0' || s === 'inf' || s === 'never' || s === 'always';
}

const CONFIG_VERSION = 1;
const DEFAULTS = {
  version: CONFIG_VERSION,
  gui: { port: 18400, token: '', openBrowser: true, appWindow: true, appWindowSize: '1120,780' },
...
function profileErrors(p, all) {
  ...
  if (!isForever(p.ttl)) { try { U.parseTtl(p.ttl || '1h'); } catch (e) { errs.push(e.message); } }
  // 目标地址：只做离线可判定的检查（IP 字面量），主机名在启动时才解析
  if (p.target) {
    try {
      const t = UPSTREAM.parseTarget(p.target, p.port);
      if (UPSTREAM.looksLikeUrl(t.host)) errs.push('target 只填 host:port，不要带 http:// 前缀');
      if (['0.0.0.0', '::'].includes(String(t.host))) errs.push('target 不能是 0.0.0.0 / ::，要填具体设备地址');
      if (require('node:net').isIPv4(t.host) && !UPSTREAM.isPrivateIPv4(t.host) && !p.allowPublicTarget) {
        errs.push('target 是公网地址 ' + t.host + '：默认只允许本机/局域网地址（确实需要请勾选"允许公网目标"）');
      }
    } catch (e) { errs.push('target 不合法: ' + e.message); }
  }
  if (String(p.mode || '') === 'named') {
    const hv = NAMED.checkHostnames(p);
    if (!hv.ok) errs.push('命名隧道需要填自有域名: ' + hv.error);
  }
...
function normalizeProfile(input, index, cfg) {
  ...
  p.mode = String(p.mode || '').toLowerCase() === 'named' ? 'named' : 'quick';
  p.target = String(p.target || '').trim();
  p.fixedHost = !!p.fixedHost;
  const rawHosts = [];
  if (Array.isArray(p.hostnames)) rawHosts.push.apply(rawHosts, p.hostnames);
  if (p.hostname) rawHosts.push(p.hostname);
  p.hostnames = NAMED.hostnamesOf({ hostnames: rawHosts });
  p.hostname = p.hostnames[0] || '';
  p.keepAlive = p.keepAlive !== false;
  p.tunnelName = String(p.tunnelName || p.id || '').trim();
  p.ttl = String(p.ttl || (p.mode === 'named' ? 'forever' : (cfg.defaults.ttl || '1h')));
