...
const TOOLS = [
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: { id: { type: 'string' }, hostname: { type: 'string', description: '如 app.example.com' }, start: { type: 'boolean', description: '绑定后是否立刻启动（默认 true）' } },
...
async function callTool(ctx, cfg, name, args) {
  ...
  if (name === 'set_custom_domain') {
    ...
    const r = S.setupDomain({ appDir: appDir, stateDir: ctx.baseDir, profile: p, hostname: String(args.hostname) });
    if (!r.ok) return errText(r.error || '绑定失败' + (r.needLogin ? '（先调用 cloudflare_login）' : ''));
    cfgLocal.updateProfile(p.id, { mode: 'named', hostname: r.hostname, tunnelId: r.tunnelId, tunnelName: r.tunnelName, ttl: 'forever' });
    let started = null;
    if (args.start !== false) {
      const d = await needDaemon();
