...
async function ensureDaemon(ctx, opts) {
  opts = opts || {};
  const baseDir = path.resolve(ctx.baseDir || path.dirname(ctx.configFile || '.'));
  const cfgPath = path.resolve(ctx.configFile || path.join(baseDir, 'config.json'));
  const found = await findDaemon(baseDir);
  if (found) return found;
  const e = entryOf(ctx.appDir);
  const extra = [];
  if (opts.noTray) extra.push('--no-tray');
  const args = e.args.concat(['gui', '--quiet', '--no-open', '--config', cfgPath]).concat(extra);
