...
function setupDomain(opts) {
  ...
  for (const hostname of hv.hostnames) {
    const routed = run(cf.path, NAMED.routeArgs({ certFile: certFile, tunnelRef: tunnelId, hostname: hostname }), { cwd: opts.appDir });
    const routeOut = routed.output || '';
    routeLogs.push(hostname + ': ' + (routeOut || ('退出码 ' + routed.status)));
    if (routed.status === 0 || /already|exists/i.test(routeOut)) { okHosts.push(hostname); continue; }
    return { ok: false, error: '绑定域名 ' + hostname + ' 失败：' + (routeOut || '（退出码 ' + routed.status + '）'), tunnelId: tunnelId, hostname: hostname, hostnames: okHosts };
  }
  return {
    ok: true, tunnelId: tunnelId, hostname: hv.hostnames[0], hostnames: hv.hostnames, tunnelName: tunnelName,
    publicUrl: 'https://' + hv.hostnames[0], publicUrls: hv.hostnames.map((h) => 'https://' + h),
    credentialsFile: credFile, certFile: certFile, routeOutput: routeLogs.join('\n'), created: !!opts.tunnelId ? false : true,
  };
}
/** 删除命名隧道（Cloudflare 侧） */
