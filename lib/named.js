...
/** 一个通道可以绑多个域名：hostnames 数组优先，hostname 作为主域名 */
function hostnamesOf(profile) {
  const out = [];
  const add = (h) => {
    const v = validateHostname(h);
    if (v.ok && out.indexOf(v.hostname) < 0) out.push(v.hostname);
  };
  if (profile && Array.isArray(profile.hostnames)) profile.hostnames.forEach(add);
  if (profile && profile.hostname) add(profile.hostname);
  return out;
}
/** 校验一组域名，返回第一个错误 */
function checkHostnames(profile) {
  const list = hostnamesOf(profile);
  if (!list.length) return { ok: false, hostnames: [], error: '还没有填自有域名（示例：app.example.com）' };
  const raw = [];
  if (profile && Array.isArray(profile.hostnames)) raw.push.apply(raw, profile.hostnames);
  if (profile && profile.hostname) raw.push(profile.hostname);
  for (const h of raw) {
    const v = validateHostname(h);
    if (!v.ok) return { ok: false, hostnames: list, error: v.error + '（域名：' + String(h).trim() + '）' };
  }
  return { ok: true, hostnames: list, error: '' };
}
...
function buildConfigYaml(opts) {
  const list = [];
  const pushUnique = (h) => { const v = validateHostname(h); if (v.ok && list.indexOf(v.hostname) < 0) list.push(v.hostname); };
  if (Array.isArray(opts.hostnames)) opts.hostnames.forEach(pushUnique);
  if (opts.hostname) pushUnique(opts.hostname);
  if (!list.length) throw new Error(validateHostname(opts.hostname || (opts.hostnames || [])[0]).error || '域名不合法');
...
function certInfo(baseDir, stateDir) {
  ...
  try {
    ...
    const info = { ok: ok, path: f, bytes: Buffer.byteLength(s), exists: true, validTo: '', daysLeft: null, expired: false };
    try {
      const m = s.match(/-----BEGIN CERTIFICATE-----[\s\S]*?-----END CERTIFICATE-----/);
      if (m) {
        const x = new (require('node:crypto').X509Certificate)(m[0]);
        info.validTo = x.validTo;
        const t = Date.parse(x.validTo);
        if (!isNaN(t)) {
          info.daysLeft = Math.floor((t - Date.now()) / 86400000);
          info.expired = t < Date.now();
        }
      }
    } catch (e) { info.certError = e && e.message ? e.message : String(e); }
    return info;
  } catch (e) {
...
function namedReadiness(baseDir, stateDir, profile, cloudflaredOverride) {
  const v = checkHostnames(profile);
  ...
  return {
    ...
    cloudflared: cf, cert: cert, credentials: cred,
    hostname: v.ok ? v.hostnames[0] : '', hostnames: v.hostnames,
    ...
    publicUrl: v.ok ? 'https://' + v.hostnames[0] : '',
    publicUrls: v.ok ? v.hostnames.map((h) => 'https://' + h) : [],
    certDaysLeft: cert.daysLeft, certValidTo: cert.validTo,
  };
}
module.exports = {
  validateHostname, checkHostnames, hostnamesOf, isNamed, cfDir, certPath, credentialsPath, configPath, userCloudflaredDir,
