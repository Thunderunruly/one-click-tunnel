...
class TunnelManager {
  ...
  async start(id, opts) {
    opts = opts || {};
    const p = this.cfg.find(id);
    if (p) this.stopping.delete(p.id);
  ...
  /** 命名隧道 + autoStart + keepAlive：意外退出就拉起来（带频率限制） */
  maybeRestart(profile, code) {
    if (this.stopping.has(profile.id)) return;
    if (!profile.keepAlive || !profile.autoStart) return;
    const key = profile.id;
    const now = Date.now();
    const rec = this.restarts.get(key) || { times: [], timer: null };
    rec.times = rec.times.filter((t) => now - t < 10 * 60 * 1000);
    if (rec.times.length >= 6) {
      this.appendLog(key, '\n自动重启次数过多（10 分钟内 6 次），已停止重试，请检查日志\n');
      this.restarts.set(key, rec);
      return;
    }
    if (rec.timer) return;
    rec.timer = setTimeout(() => {
      ...
      this.restarts.set(key, rec);
      this.appendLog(key, '\n守护：自动重新启动（第 ' + rec.times.length + ' 次，上次退出码 ' + code + '）\n');
      this.start(profile.id, { noWait: true }).catch(() => {});
    }, 5000);
  ...
  async stop(id, opts) {
    ...
    const p = this.cfg.find(id);
    if (!p) throw new Error('找不到通道: ' + id);
    this.stopping.add(p.id);
    const rrec = this.restarts.get(p.id);
    if (rrec && rrec.timer) { clearTimeout(rrec.timer); rrec.timer = null; this.restarts.set(p.id, rrec); }
