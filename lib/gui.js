    ...
    try {
      ...
      if (pathname.startsWith('/api/')) {
        ...
        if (pathname === '/api/state' && req.method === 'GET') {
          return json(res, 200, {
            ...
            cloudflare: (function () {
              ...
              return { loggedIn: cert.ok, certFile: cert.path, namedCount: namedProfiles.length, cloudflared: NAMED.cloudflaredStatus(appDir).ok };
            })(),
          });
        }
        if (pathname === '/api/cloudflare' && req.method === 'POST') {
          ...
          if (action === 'login') {
            const r = CF.loginCloudflare({ appDir: appDir, stateDir: baseDir });
            return json(res, r.ok ? 200 : 400, { ok: r.ok, error: r.error || '', certFile: r.certFile || '', cert: NAMED.certInfo(appDir, baseDir) });
          }
          if (action === 'status') {
