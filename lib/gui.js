...
async function runGui(opts) {
    ...
    try {
      ...
      if (pathname.startsWith('/api/')) {
        ...
        if (pathname === '/api/settings' && req.method === 'POST') {
          ...
          return json(res, 200, { ok: true, gui: cfg.data.gui, mcp: cfg.data.mcp, tray: cfg.data.tray, defaults: cfg.data.defaults });
        }
  ...
  if (opts.open !== false && cfg.data.gui.openBrowser !== false) openBrowser(guiUrl);
